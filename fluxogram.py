...
class Fluxogram:
    ...
    def __init__(self, max_flux, max_storage, grid_size = 20, storages = None, 
        ...
        initilalizes a fluxogram. must be called with:
            ...
            - fluxes: all the fluxes the fluxogram has (usually empty to begin
                        with)    
            - root: if True the square root of all data is used to get a 
                    bette representation if the span between high and low 
                    values is large
        """
        if storages == None:
            self.storages = []
        ...
        self.below_0 = False
        if self.root == True:
            self.max_flux = math.sqrt(self.max_flux)
            self.max_storage = math.sqrt(self.max_storage)
            self.grid_size = math.sqrt(self.grid_size)
    ...
    def draw(self, day = -1):
        ...
        for flux in self.fluxes:
            amount = flux.amount
            # take the squareroot if needed for better representation
            if self.root == True:
                amount = math.sqrt(amount)
            # scale the amount
            scaled_amount_flux = self.scaler(amount, self.max_flux)
...
#                     fontsize = self.grid_size * 0.3)
        ...
        for storage in self.storages:
            ...
            if amount < 0:
                amount = 0
                # needed as flag to alert user of negative values
                self.below_0 = True            
            
            if self.root == True:
                amount = math.sqrt(amount)            
            # scale the amount
            scaled_amount_stor = self.scaler(amount, self.max_storage)
            # if the amount is 0, the diagram does draw nothing and the whole
            # diagram is shifted, to avoid this the value is set to slightly
            # above 0
            if scaled_amount_stor == 0:
                scaled_amount_stor = 0.0001
            # change_x and y, so the storages are centered to the middle
            # of their position and not to upper left. This is done by
            # adjusting it by the percentage it is off in comparison
            # to the maximal amount for storages
            x = (storage.x + (1 - amount / self.max_storage) * 0.5 
                 * self.grid_size)
            y = (storage.y - (1 - amount / self.max_storage) * 0.5 
                 * self.grid_size)
            ...
            # if root is True avoid too little fontsize
            if self.root == True:
                fontsize = self.grid_size ** 2 * 0.7
            plt.text(storage.x + self.grid_size * 0.1, 
                     storage.y - self.grid_size * 1.2, storage.name, 
                     fontsize = fontsize)
            # draw a date if the fluxogram is to be animated
            if day > -1:
    ...
    def animate(self, timeseries_fluxes, timeseries_storages, anim_name):
        ...
        try:
            ...
            print("Old video with same name deleted")
        ...
        if self.below_0 == True:
            print("Value(s) below 0 found. Set to 0 for convience. Check data")
    ...
    def scaler(self, value_in, base_max):
        ...
        new_val =  (((limit_max - limit_min) * (value_in - base_min)
...
if __name__ == "__main__":
    ...
    fl = Fluxogram(100, 150, grid_size = 10, root = True)
